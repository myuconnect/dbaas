...
class OracleGlobals(object, metaclass=Singleton):
	def __init__(self):
		...
		self.SQLS = {
			# objects/schema
			"isSchemaObjExists" : {
				"default" :  "SELECT object_name FROM dba_objects WHERE owner = UPPER(:OWNER) AND object_name = UPPER(:OBJECT_NAME) AND object_Type = UPPER(:OBJECT_TYPE)", 
				"criteria" : ["OWNER","OBJECT_NAME","OBJECT_TYPE"], 
				"return" : "EXISTENCE",
				"sqlType" : "select"
			},
			"getSchemaObj" : {
				"default" :  "SELECT object_type, object_name FROM dba_objects WHERE owner = UPPER(:OWNER)", 
				"criteria" : ["OWNER"], 
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getObjectDetails" :{
				"default" : "SELECT * FROM DBA_OBJECTS WHERE OWNER = UPPER(:OWNER) AND OBJECT_TYPE = UPPER(:OBJECT_TYPE) AND OBJECT_NAME = UPPER(:OBJECT_NAME)", 
				"criteria" : ["OWNER","OBJECT_TYPE","OBJECT_NAME"], 
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getObjectSizeMB" : {
				"default" : "SELECT SUM(bytes)/(1024*1024) FROM DBA_SEGMENTS WHERE owner = UPPER(:OWNER) AND object_type = UPPER(:OBJECT_TYPE) AND object_name = UPPER(:OBJECT_NAME)",
				"criteria" : ["OWNER","OBJECT_TYPE","OBJECT_NAME"], 
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getAllUserObj" : {
				"default" : "SELECT * FROM dba_objects WHERE owner = UPPER(:OWNER)",
				"criteria" : ["OWNER"], 
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			"getAllUserObjByType" : {
				"default" : "SELECT * FROM dba_objects WHERE owner = UPPER(:OWNER) AND object_type = UPPER(:OBJECT_TYPE)",
				"criteria" : ["OWNER","OBJECT_TYPE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getAllUserObjSummary" : {
				"default" : "SELECT object_type, status, count(*) as TOTAL FROM dba_objects WHERE owner = UPPER(:OWNER) group by object_type, status",
				"criteria" : ["OWNER"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getAllUserObjSummaryByStatus" : {
				"default" : "SELECT object_type, count(*) as TOTAL FROM dba_objects WHERE owner = UPPER(:OWNER) group by object_type",
				"criteria" : ["OWNER","STATUS"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getUserObjCnt" : {
				"default" : "SELECT COUNT(*) as TOTAL FROM dba_objects WHERE owner = UPPER(:OWNER)",
				"criteria" : ["OWNER"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			"getUserObjectsByStatus" : {
				"default" : "SELECT OBJECT_TYPE, OBJECT_NAME FROM dba_objects WHERE owner = UPPER(:OWNER) AND status = UPPER(:STATUS)",
				"criteria" : ["OWNER","STATUS"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getSchemaSizeMB" : {
				"default" : "SELECT SUM(bytes)/(1024*1024) as SIZE_MB FROM DBA_SEGMENTS WHERE owner = UPPER(:OWNER)",
				"criteria" : ["OWNER"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			# roles/privilege
			"isRoleExists" : {
				"default" : "SELECT role FROM dba_roles WHERE role = UPPER(:ROLE)",
				"criteria" : ["ROLE"],
				"return" : "EXISTENCE",
				"sqlType" : "select"
			},
			"getRoleInfo" : {
				"default" : "SELECT * FROM dba_roles WHERE role = UPPER(:ROLE)",
				"criteria" : ["ROLE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			"getAllUserRole" : {
				"default" :  "SELECT role as ROLE FROM role_tab_privs WHERE owner = UPPER(:OWNER)",
				"criteria" : ["OWNER"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getSysPrivsGrantedTo" : {
				"default" : "SELECT * FROM dba_sys_privs WHERE grantee = UPPER(:GRANTEE)",
				"criteria" : ["GRANTEE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getTabPrivsGrantedTo" : {
				"default" : "SELECT * FROM dba_tab_privs WHERE grantee = UPPER(:GRANTEE)",
				"criteria" : ["GRANTEE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getRoleGrantedTo" : {
				"default" : "SELECT * FROM dba_role_privs WHERE grantee = UPPER(:GRANTEE)",
				"criteria" : ["GRANTEE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			"getTBSDBFiles" : {"default" : "SELECT * FROM dba_datafiles WHERE tablespace_name = UPPER(:TABLESPACE_NAME)",
				...
				"default" :  """
					SELECT tablespace_name, SUM(dbfiles.bytes)/(1024*1024) AS SIZE_MB, SUM(free.bytes)/(1024*1024*1024) AS FREE_MB
						FROM dba_data_files dbfiles, 
							dba_free_space free
						WHERE dbfiles.tablespace_name = DECODE(UPPER(:TABLESPACE_NAME),'ALL',dbfiles.tablespace_name, UPPER(:TABLESPACE_NAME)) 
			...
			"getUserSessionDetail": {
				"default" : "SELECT * FROM v$session where username = UPPER(:USERNAME)",
				"criteria" : ["USERNAME"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getUserSessionDetailByStatus": {
				"default" : "SELECT * FROM v$session where username = UPPER(:USERNAME) AND status = UPPER(:STATUS)",
				"criteria" : ["USERNAME","STATUS"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getSessCntByUserStatus": {
				"default" : "SELECT count(*) as TOTAL FROM v$session where status = UPPER(:STATUS) AND username = UPPER(:USERNAME)",
			...
			"getSessCntByUserMachine": {
				"default" : "SELECT status, count(*) as TOTAL FROM v$session where username = UPPER(:USERNAME) AND machine = :MACHINE",
				"criteria" : ["MACHINE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			...
			# source code
			"getDDL" : {
				"default" : "SELECT DBMS_METADATA.GET_DDL(UPPER(:OBJECT_TYPE), UPPER(:OBJECT_NAME), UPPER(:OWNER)) AS SOURCE_CODE FROM DUAL",
				"criteria" : ["OWNER","OBJECT_TYPE","OBJECT_NAME"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getDependentDDL" : {
				"default" : "SELECT DBMS_METADATA.GET_DEPENDENT_DDL(UPPER(:OBJECT_TYPE), UPPER(:OBJECT_NAME), UPPER(:OWNER)) AS SOURCE_CODE FROM DUAL",
				"criteria" : ["OWNER","OBJECT_TYPE","OBJECT_NAME"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getGrantedDDL" : {
				"default" : "SELECT DBMS_METADATA.GET_GRANTED_DDL(UPPER(:GRANT_TYPE), UPPER(:GRANTEE)) AS SOURCE_CODE  FROM DUAL",
			...
			# user/profile
			"isUserExists" : {
				"default" : "SELECT username FROM dba_users WHERE username = UPPER(:USER_NAME)",
				"criteria" : ["USER_NAME"],
				"return" : "EXISTENCE",
				"sqlType" : "select"
			},
			"isProfileExists" : {
				"default" : "SELECT PROFILE FROM dba_profiles WHERE profile = UPPER(:PROFILE)",
				"criteria" : ["PROFILE"],
				"return" : "EXISTENCE",
				"sqlType" : "select"
			},
			"isSysPrivsGrantedTo" : {
				"default" : "SELECT privilege FROM dba_sys_privs WHERE GRANTEE = :GRANTEE AND privilege = UPPER(:PRIVILEGE)",
				"criteria" : ["GRANTEE","PRIVILEGE"],
				"return" : "EXISTENCE",
				"sqlType" : "select"
			},
			"isObjPrivsGrantedTo" : {
				"default" : "SELECT privilege FROM dba_tab_privs WHERE GRANTEE = :GRANTEE AND owner = UPPER(:OWNER) AND object_name = UPPER(:OBJECT_NAME) and privilege = UPPER(:PRIVILEGE)",
			...
			"getUserInfo" : {
				"default" : "SELECT * FROM dba_users WHERE username = UPPER(:USER_NAME)",
				"criteria" : ["USER_NAME"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getUserExtInfo" : {
				"default" : "SELECT * FROM sys.user$ WHERE name = UPPER(:USER_NAME)",
				"criteria" : ["USER_NAME"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
			"getProfileInfo" : {
				"default" : "SELECT * FROM dba_profiles WHERE profile = UPPER(:PROFILE)",
				"criteria" : ["PROFILE"],
				"return" : "RESULT",
				"sqlType" : "select"
			},
		...
		self.SELECT_LIMIT_ROWS = 100 # limits selected data need to be logged 
		self.CALL_TIMEOUT = 0 # this value is in milliseconds for call timeout
		# these sqls will be run for each oracle connection established

		self.GLOBAL_INIT_DB_SQLS = [
			{"comp" : "DB", "sql" : self.SQLS["getDBDetails"]["default"], "sqlArg" : {}}
		]

		"""
		self.GLOBAL_INIT_DB_SQLS = [
			{"comp" : "DB", "sql" : self.getSysTimeStamp, "sqlArg" : {}}
		]
		"""
		
		self.DBDOC_SERVER_DB = "oltp55"
		# commenting this out as this is adding time in connection object due to procedure call, moving this as a stand alone method (this is only needed for inactive admin list) 
		"""
		self.INIT_DB_PROC = {
			self.DBDOC_SERVER_DB : [
				{
					"step" : 1,
					"callType" : "procedure",
					"procedure" : "db_mon_owner.maintain_report_data.pop_user_proc_date", 
					"comment" : "sysdate can be backdated if needed",
					"param" : {"p_proc_date" : self.util.getCurrentDate(), "p_opco" : "MARSH"}
				},
				{
					"step" : 2,
					"callType" : "procedure",
					"procedure" : "db_mon_owner.maintain_report_data.pop_db_proc_date",
					"param" : {"p_proc_date" : self.util.getCurrentDate()} # for all opcos
				}
			]
		}
		"""
